...
class TsDslGenerator implements IGenerator {
	...
	def dispatch constraintcode( FeatureTypeConstraint tc, EClass clazz ) '''
		ensureFeatureType( «clazz.getter», «tc.feature.getter»«tc.types.fold("",[s, e|s+", "+e.code])» );
	'''

	def dispatch constraintcode( ComatibilityTypeConstraint tc, EClass clazz ) {
		switch (tc.op) {
			SameTypeOp: "TODO currently not yet suported"
			SubTypeOp: '''ensureOrderedCompatibility( «clazz.getter», «tc.feature1.getter», «tc.feature2.getter» );'''
			UnoderedSubtypeOp: '''ensureUnorderedCompatibility( «clazz.getter», «tc.feature1.getter», «tc.feature2.getter» );'''
		}
	}
