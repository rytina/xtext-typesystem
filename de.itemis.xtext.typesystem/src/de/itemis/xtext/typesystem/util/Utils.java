...
public class Utils {
	 ...
	 */
	public static <C extends EObject> C ancestor(EObject ctx,
			Class<C> ancestorClass) {
	...
	public static String name(EObject object) {
		EStructuralFeature namefeature = object.eClass().getEStructuralFeature(
				"name");
		if (namefeature == null) {
			return null;
		} else {
	...
	/**
	 * a helper class for scope construction that contains a collection of
	 * elements as well as a function that calculates the name for the object.
	 * Used with buildScope below
	 */
	...
	/**
	 * utility class that finds all instances of a given type in the index. Note
	 * that the client instantiating this class has to pass in an
	 * IResourceDescriptions, which the client may get using @Inject
	 */
	public static class GlobalFinder {
		...
		public IScope find(Filter p, EClass... classes) {
			...
			Iterable<IResourceDescription> allResourceDescriptions = global
					.getAllResourceDescriptions();
			for (IResourceDescription description : allResourceDescriptions) {
				for (EClass cls : classes) {
					Iterable<IEObjectDescription> enums = description
							.getExportedObjectsByType(cls);
					for (IEObjectDescription od : enums) {
						if (p == null)
							res.add(od);
						else if (p.include(od))
							res.add(od);
					}
				}
			}
	...
	/**
	 * outputs a more or less useful string represntation for an EObject
	 */
	...
	public static Object eget(EObject eObject, String featureName) {
		Object val = eObject.eGet(eObject.eClass().getEStructuralFeature(
				featureName));
		return val;
	}

	public static EObject egetAndResolve(EObject eObject, String featureName,
			ResourceSet rs) {
