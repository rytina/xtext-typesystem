...
public abstract class DefaultTypesystem implements ITypesystem {
	...
	/**
	 * the polymorphic dispatcher (see Xtext docs) used to call a type method
	 * polymorhpically for various types of elements. Use EObject type(YourType
	 * element) in subclass to define custom type calculation rules.
	 */
	...
	private final PolymorphicDispatcher<Boolean> compareTypeDispatcher = new PolymorphicDispatcher<Boolean>(
			"compareTypes", 4, 4, Collections.singletonList(this), new ErrorHandler<Boolean>() {
				public Boolean handle(Object[] params, Throwable e) {
					return null;
				}
			});

	private final PolymorphicDispatcher<String> typeToStringDispatcher = new PolymorphicDispatcher<String>(
			"typeToString", 1, 1, Collections.singletonList(this), new ErrorHandler<String>() {
				public String handle(Object[] params, Throwable e) {
					return null;
				}
			});

	private final PolymorphicDispatcher<EObject> typeCoercionDispatcher = new PolymorphicDispatcher<EObject>(
			"typeCoerce", 4, 4, Collections.singletonList(this), new ErrorHandler<EObject>() {
				public EObject handle(Object[] params, Throwable e) {
					return null;
				}
			});
	...
	/**
	 * executes the type system rules declated via the doInitialize() method.
	 */
	private EObject useDeclarativeRules(EObject element, TypeCalculationTrace trace) {
		...
		if (applicableRules.size() == 0) {
			LOGGER.warn("no type calculation rules found for EClass " + eString(element) + " (isProxy="
					+ element.eIsProxy() + ")");
	...
	public EObject computeCommonType(EObject element1, EObject type1, EObject element2, EObject type2,
			TypeCalculationTrace trace) {
		if (isSameType(element1, type1, element2, type2, trace))
			return type1;
		if (isSubtype(type1.eClass(), type2.eClass()))
			return type2;
		if (isSubtype(type2.eClass(), type1.eClass()))
			return type1;

		EObject coercedType1 = tryToCoerceType(element1, type1, type2, trace);
		if (coercedType1 != null) {
			if (isSameType(element1, coercedType1, element2, type2, trace))
				return type1;
			if (isSubtype(coercedType1.eClass(), type2.eClass()))
				return type2;
			if (isSubtype(type2.eClass(), coercedType1.eClass()))
				return type1;
		}

		EObject coercedType2 = tryToCoerceType(element2, type2, type1, trace);
		if (coercedType2 != null) {
			if (isSameType(element1, type1, element2, coercedType2, trace))
				return type1;
			if (isSubtype(type1.eClass(), coercedType2.eClass()))
				return type2;
			if (isSubtype(coercedType2.eClass(), type1.eClass()))
				return type1;
		}
	...
	public EObject tryToCoerceType(EObject element, EObject currentType, EObject requiredType,
			TypeCalculationTrace trace) {
	...
	/**
	 * call this method to find out the type of an element. Lazily calls
	 * initialize() to register the declarative typing rules. Uses the poly
	 * dispatcher to call a type method (defined in subclasses) and if nothing
	 * is found, falls back to useDeclarativeRules
	 */
	...
	private void checkSuperSuperTypes(EObject candidateType) throws InvalidType {
		if (typeRootEClasses.size() == 0)
			return;
		for (EClass ec : typeRootEClasses) {
			if (hasValidSuperSuperType(candidateType, ec))
				return;
		}
		throw new InvalidType("invalid type EClass: " + typeString(candidateType) + " is not a subtype of "
				+ typeStrings(typeRootEClasses.toArray(new EClass[] {}))
				+ ", which have been registered as the required super types of all calculated types.");
	}
	...
	/**
	 * to be overridden in subclasses; used to define the declarative typing
	 * rules
	 */
	protected abstract void initialize();

	/**
	 * checks if the same class is registered for type computation more than
	 * once; this is currently not possible.
	 */
	...
	/**
	 * registers a type comparison feature. If two types (i.e. instances of cls)
	 * are compared using hasSameType, then the values of these features are
	 * also compared. Typically called from initialize.
	 * 
	 * @throws EClassDoesntHaveFeatureException
	 */
	protected void declareTypeComparisonFeature(EClass cls, EStructuralFeature feature)
			throws FeatureMustBeSingleValuedException, DuplicateRegistrationException, EClassDoesntHaveFeatureException {
		checkSuperSuperTypes(cls);
		ensureValidFeature(cls, feature);
		typeComparisonFeatures.put(cls, feature);
	}

	private void ensureValidFeature(EClass cls, EStructuralFeature feature) throws FeatureMustBeSingleValuedException,
			EClassDoesntHaveFeatureException {
		if (feature.isMany()) {
		...
		if (!classHasFeature(cls, feature)) {
			throw new EClassDoesntHaveFeatureException("EClass " + cls.getName() + " does not have a feature "
					+ feature.getName());
		}
	}
	...
	protected void declareTypeRecursionFeature(EClass cls, EStructuralFeature feature)
			throws FeatureMustBeSingleValuedException, DuplicateRegistrationException, EClassDoesntHaveFeatureException {
		checkSuperSuperTypes(cls);
		ensureValidFeature(cls, feature);
	...
	/**
	 * calculate type by cloning the element itself to get the type Typically
	...
	/**
	 * use a new instance of the associated class as the type Typically called
	 * from initialize.
	 */
	...
	/**
	 * derive the type of the element from the (hopefully defined :-)) type of
	 * one of its features Typically called from initialize.
	 * 
	 * @throws EClassDoesntHaveFeatureException
	 * @throws FeatureMustBeSingleValuedException
	 */
	protected void useTypeOfFeature(EClass elemClass, EStructuralFeature feature)
			throws DuplicateRegistrationException, EClassDoesntHaveFeatureException, FeatureMustBeSingleValuedException {
		checkDuplicateRegistration(elemClass);
	...
	/**
	 * derive the type of the elment by calculating the common type of the two
	 * features.
	 * 
	 ...
	 * @throws FeatureMustBeSingleValuedException
	 */
	protected void computeCommonType(EClass elemClass, EStructuralFeature f1, EStructuralFeature f2)
			throws DuplicateRegistrationException, EClassDoesntHaveFeatureException, FeatureMustBeSingleValuedException {
		checkDuplicateRegistration(elemClass);
	...
	/**
	 * move up the containment hierachy to derive the type from an ancestor
	...
	/**
	 * compare the two types. First make sure the two classes are the same Then,
	 * if a type comparison feature is registered, compare the value of the
	 * feature for both elements
	 */
	public boolean isSameType(EObject element1, EObject type1, EObject element2, EObject type2,
			TypeCalculationTrace trace) {
	 ...
	 */
	private boolean handleComparisonAndRecursionFeatures(EObject type1, EObject type2, CheckKind kind,
			TypeCalculationTrace trace) {
		...
		if (typeRecursionFeatures.containsKey(type1EClass)) {
			...
			for (EStructuralFeature f : recursionFeatures) {
				...
				if (kind == CheckKind.same) {
					if (!isSameType(val1, val1type, val2, val2type, trace))
						return false;
				} else if (kind == CheckKind.unordered) {
					if (!isCompatibleTypeUnordered(val1, val1type, val2, val2type, trace))
						return false;
				} else if (kind == CheckKind.ordered) {
					if (!isCompatibleTypeOrdered(val1, val1type, val2, val2type, trace))
						return false;
				}
			}
		}
	...
	/**
	 * returns true if the type of element1 has a type that's a super type of
	 * element2's type -- or vice versa TODO: Possibility to do a polymorphic
	 * dispatcher override
	 */
	public boolean isCompatibleTypeUnordered(EObject element1, EObject type1, EObject element2, EObject type2,
			TypeCalculationTrace trace) {
	...
	/**
	 * returns true if the type of element1 and element2 are the same, or
	 * element2 is a subtype of element1
	 */
	public boolean isCompatibleTypeOrdered(EObject element1, EObject type1, EObject element2, EObject type2,
			TypeCalculationTrace trace) {
	...
	private boolean isSubtype(EClass subtypeClass, EClass supertypeClass) {
		...
		if (subtypeSupertypes.contains(supertypeClass))
			return true;
		return false;
	}
	...
	private boolean isSameOrSubtype(EClass subtypeClass, EClass supertypeClass, TypeCalculationTrace trace) {
		List<EClass> subtypeSupertypes = allSupertypes(subtypeClass);
		if (subtypeSupertypes.contains(supertypeClass) || subtypeClass == supertypeClass)
			return true;
		return false;
	}
	...
	public boolean isInstanceOf(EObject actualType, EClass expectedType, TypeCalculationTrace trace) {
		...
		for (EClass superType : allSubtypes) {
			if (superType.isInstance(actualType))
				return true;
		}
		return false;
	}
	...
	/**
	 * checks whether element's name is unique in the collection passed in.
	 */
	...
	/**
	 * define a type constraint for a feature. The type of feature of instances
	 * of ctxClass must be an instance of typeClass. Typically called from
	 * initialize.
	 * 
	 * @throws EClassDoesntHaveFeatureException
	 * @throws InvalidTypeSpecification
	 */
	protected void ensureFeatureType(EClass ctxClass, EStructuralFeature feature, Object... validTypes)
			throws FeatureMustBeSingleValuedException, EClassDoesntHaveFeatureException, InvalidTypeSpecification {
	 ...
	 */
	protected void ensureFeatureType(String errorMessage, EClass ctxClass, EStructuralFeature feature,
			Object... validTypes) throws FeatureMustBeSingleValuedException, EClassDoesntHaveFeatureException,
		...
		for (Object o : validTypes) {
			if (!(o instanceof EClass) && !(o instanceof StaticCustomTypeChecker) && !(o instanceof TypeCharacteristic)) {
				throw new InvalidTypeSpecification(
						"types must be EClasses or instances of CustomTypechecker or instances of TypeCharacteristic");
			}
	 ...
	 */
	protected void ensureFeatureType(IErrorMessageProvider errorMessageProvider, EClass ctxClass,
			EStructuralFeature feature, Object... validTypes) throws FeatureMustBeSingleValuedException,
			EClassDoesntHaveFeatureException, InvalidTypeSpecification {
		ensureValidFeature(ctxClass, feature);
		for (Object o : validTypes) {
			if (!(o instanceof EClass) && !(o instanceof StaticCustomTypeChecker) && !(o instanceof TypeCharacteristic)) {
				throw new InvalidTypeSpecification(
	 ...
	 * @throws EClassDoesntHaveFeatureException
	 */
	protected void ensureUnorderedCompatibility(EClass ctxClass, EStructuralFeature feature1,
			EStructuralFeature feature2) throws FeatureMustBeSingleValuedException, EClassDoesntHaveFeatureException {
	...
	protected void ensureUnorderedCompatibility(String errorMessage, EClass ctxClass, EStructuralFeature feature1,
			EStructuralFeature feature2) throws FeatureMustBeSingleValuedException, EClassDoesntHaveFeatureException {
		ensureValidFeature(ctxClass, feature1);
		ensureValidFeature(ctxClass, feature2);
	...
	protected void ensureUnorderedCompatibility(EClass ctxClass, EStructuralFeature feature2)
			throws FeatureMustBeSingleValuedException, EClassDoesntHaveFeatureException {
		ensureValidFeature(ctxClass, feature2);
		singleElementChecks.add(new EnsureUnorderedCompatibilityCheck(ctxClass, feature2));
	}

	protected void ensureUnorderedCompatibility(String errorMessage, EClass ctxClass, EStructuralFeature feature2)
			throws FeatureMustBeSingleValuedException, EClassDoesntHaveFeatureException {
		ensureValidFeature(ctxClass, feature2);
	...
	/**
	 * define a compatibility constraint for two features of an EClass.
	 * Typically called from initialize. feature2 must be the same or a subtype
	 * of feature 1
	 * 
	 * @throws EClassDoesntHaveFeatureException
	 */
	protected void ensureOrderedCompatibility(EClass ctxClass, EStructuralFeature left, EStructuralFeature right)
			throws FeatureMustBeSingleValuedException, EClassDoesntHaveFeatureException {
	...
	protected void ensureOrderedCompatibility(String errorMessage, EClass ctxClass, EStructuralFeature left,
			EStructuralFeature right) throws FeatureMustBeSingleValuedException, EClassDoesntHaveFeatureException {
	...
	protected void ensureOrderedCompatibility(IErrorMessageProvider msgProvider, EClass ctxClass,
			EStructuralFeature left, EStructuralFeature right) throws FeatureMustBeSingleValuedException,
			EClassDoesntHaveFeatureException {
		checkSuperSuperTypes(ctxClass);
		ensureValidFeature(ctxClass, left);
		ensureValidFeature(ctxClass, right);
		singleElementChecks.add(new EnsureOrderedCompatibilityCheck(msgProvider, ctxClass, left, right));
	}

	protected void ensureOrderedCompatibility(EClass ctxClass, EStructuralFeature right)
			throws FeatureMustBeSingleValuedException, EClassDoesntHaveFeatureException {
		checkSuperSuperTypes(ctxClass);
		ensureValidFeature(ctxClass, right);
	...
	protected void ensureOrderedCompatibility(String errorMessage, EClass ctxClass, EStructuralFeature right)
			throws FeatureMustBeSingleValuedException, EClassDoesntHaveFeatureException {
		checkSuperSuperTypes(ctxClass);
		ensureValidFeature(ctxClass, right);
	...
	/**
	 * called from a validator to execute the type system constraints
	 */
	...
	public String typeString(Object type) {
		...
		if (type instanceof EClass) {
			...
			// if ( name.toLowerCase().endsWith("type")) name =
			...
			// if ( ecname.toLowerCase().endsWith("type")) ecname =
			// ecname.substring(0,ecname.length()-4);
			if (inner.equals(""))
				return ecname;
			return ecname + "(" + inner + ")";
		} else {
